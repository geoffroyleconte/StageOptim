...
for ac in [0, 1, 2]#, :minres_qlp]#, :minares]
  ...
  stats1 = RipQP.ripqp(qm, iconf = RipQP.InputConfig(
                          ...
                          solve_method=formul, history=true, analytic_center = ac,
  ...
  str2 = " K2 analytic center "
  plot!(prc, stats1.solver_specific[:rcNormH], label = string("rcNorm ", str2, ac))
  plot!(prb, stats1.solver_specific[:rbNormH], label = string("rbNorm ", str2, ac))
  plot!(ppdd, stats1.solver_specific[:pddH], label = string("pdd ", str2, ac))
  plot!(pnp, stats1.solver_specific[:nprodH], label = string("nprod ", str2, ac))
  plot!(pμ, stats1.solver_specific[:μH], label = string("μ ", str2, ac))
  plot!(pmbd, stats1.solver_specific[:min_bound_distH], label = string("mbd ", str2, ac))
  zeros_logscale!(stats1.solver_specific[:KresNormH], min_val)
  plot!(pkrN, stats1.solver_specific[:KresNormH], label = string("krN ", str2, ac))
  ...
  str2 = " K2.5 analytic center "
  plot!(prc, stats2.solver_specific[:rcNormH], label = string("rcNorm ", str2, ac))
  plot!(prb, stats2.solver_specific[:rbNormH], label = string("rbNorm ", str2, ac))
  plot!(ppdd, stats2.solver_specific[:pddH], label = string("pdd ", str2, ac))
  plot!(pnp, stats2.solver_specific[:nprodH], label = string("nprod ", str2, ac))
  plot!(pμ, stats2.solver_specific[:μH], label = string("μ ", str2, ac))
  plot!(pmbd, stats2.solver_specific[:min_bound_distH], label = string("mbd ", str2, ac))
  zeros_logscale!(stats2.solver_specific[:KresNormH], min_val)
  plot!(pkrN, stats2.solver_specific[:KresNormH], label = string("krN ", str2, ac))
end
plot!(pnp)
