...
function ripqpK2DQGMRES5(qm)
  return RipQP.ripqp(qm, display = true, iconf = RipQP.InputConfig(
                    sp = RipQP.K2KrylovParams(kmethod=:dqgmres, atol_min=1.0e-10, rtol_min=1.0e-10, memory=5), 
...
function ripqpK2DQGMRES10(qm)
  return RipQP.ripqp(qm, display = true, iconf = RipQP.InputConfig(
                    sp = RipQP.K2KrylovParams(kmethod=:dqgmres, atol_min=1.0e-10, rtol_min=1.0e-10, memory=10), 
                    solve_method=:IPF, history=true,
                    # w = RipQP.SystemWrite(write=true, kfirst=1, name = string(save_path, "\\CVXQP1_M"), kgap=1000)), 
                    ),
                itol = RipQP.InputTol(max_iter=50, max_time=20.0))
...
function ripqpK2DQGMRES50(qm)
  return RipQP.ripqp(qm, display = true, iconf = RipQP.InputConfig(
                    sp = RipQP.K2KrylovParams(kmethod=:dqgmres, atol_min=1.0e-10, rtol_min=1.0e-10, memory=50), 
                    solve_method=:IPF, history=true,
                    # w = RipQP.SystemWrite(write=true, kfirst=1, name = string(save_path, "\\CVXQP1_M"), kgap=1000)), 
                    ),
                itol = RipQP.InputTol(max_iter=50, max_time=20.0))
...
function ripqpK2DQGMRES100(qm)
  return RipQP.ripqp(qm, display = true, iconf = RipQP.InputConfig(
                    sp = RipQP.K2KrylovParams(kmethod=:dqgmres, atol_min=1.0e-10, rtol_min=1.0e-10, memory=100), 
                    solve_method=:IPF, history=true,
                    # w = RipQP.SystemWrite(write=true, kfirst=1, name = string(save_path, "\\CVXQP1_M"), kgap=1000)), 
                    ),
                itol = RipQP.InputTol(max_iter=50, max_time=20.0))
...
N = ones(n_pb)
solvers = [:ripqpK2MINRES, :ripqpK2DQGMRES5, :ripqpK2DQGMRES10, :ripqpK2DQGMRES50, :ripqpK2DQGMRES100]
for is in 1: length(solvers)
  optimize_ripqp!(path_pb, eval(solvers[is]), n_pb, data, is)
end

perf = data_profile(PlotsBackend(), data, N, [string(solver) for solver in solvers], legend=:topleft,
                    τ= 1.0e-2)
# plot(perf, )
