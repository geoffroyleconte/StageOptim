...
function gurobi2(QM)
    return gurobi(QM, presolve=0, crossover=0, display=0, threads=1)
end
...
file_lp = jldopen(string(save_path, "/gurobi_scaling_lp4.jld2"), "w")
file_lp["stats"] = problems_stats_lp
close(file_lp)

problems_stats_qp =  optimize_gurobi(path_pb_qp)

file_qp = jldopen(string(save_path, "/gurobi_scaling_qp4.jld2"), "w")
