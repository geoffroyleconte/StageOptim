...
function equilibrationRT(A, n_rows, n_cols)
    ...
    for i=1:n_rows
        A_i = A[i,:]
#         nnz_i = A_i[A_i.!=0.]
#         r[i] = 1/sqrt(maximum(abs.(nnz_i))*minimum(abs.(nnz_i)))
        r_i_inv = maximum(abs.(A_i))
        if r_i_inv != 0.
            r[i] = 1/r_i_inv
        end
    end
    for j=1:n_cols
        A_j = A[:,j]
#         nnz_j = A_j[A_j.!=0.]
#         t[j] = 1/sqrt(maximum(abs.(nnz_j))*minimum(abs.(nnz_j)))
        t_j_inv = maximum(abs.(A_j))
        if t_j_inv != 0.
            t[j] = 1/t_j_inv
        end
    end
...
function mehrotraPCQuadBounds(QM; max_iter=200, ϵ_pdd=1e-8, ϵ_rb=1e-6, ϵ_rc=1e-6,
                              tol_Δx=1e-16, ϵ_μ=0., max_time=120., scaling=true,
                              display=true)
...
path_pb = "/user/eleves/gleconte2017/Z/Documents/TFE/netlib"
...
stats2 =  mehrotraPCQuadBounds(SM2)  # compile code
 ...
 function optimize_netlib_mehrotra(path_netlib)
     problems = []
     i_max = 1000
     i = 1
...
problems_stats =  optimize_netlib_mehrotra(path_pb)
 ...
 open(string(save_path, "/mehrotra_ldl2_scaling.tex"), "w") do io
   println(io, "\\documentclass[varwidth=20cm,crop=true]{standalone}")
   println(io, "\\usepackage{longtable}")
   println(io, "\\begin{document}")
   latex_table(io, problems_stats)
   println(io, "\\end{document}")
 end


 jldopen(string(save_path, "/mehrotra_ldl2_scaling.jld2"), "w") do file
