...
stats =  ripqp(qm, mode=:mono, K=0)  # compile code

function ripqp_bm(QM)
    return ripqp(QM, mode=:mono, K=0)
end
...
file_lp = jldopen(string(save_path, "/ripqp_lp_mono_c0.jld2"), "w")
file_lp["stats"] = problems_stats_lp
close(file_lp)

problems_stats_qp =  optimize_ripqp(path_pb_qp)

file_qp = jldopen(string(save_path, "/ripqp_qp_mono_c0.jld2"), "w")
