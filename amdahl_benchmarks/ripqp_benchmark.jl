...
ripqp_bm_classic(QM) = ripqp(QM, itol = InputTol(max_time=1200.) , iconf = InputConfig(solve_method=:IPF))
# ripqp_bm_multi(QM) = ripqp(QM, iconf = InputConfig(mode=:multi), itol = InputTol(max_time=1200.))
# ripqp_bm_ref(QM) = ripqp(QM, iconf = InputConfig(refinement = :ref), itol = InputTol(max_time=1200.))
# ripqp_bm_multiref(QM) = ripqp(QM, iconf = InputConfig(mode=:multi, refinement = :multiref), itol = InputTol(max_time=1200.))
# ripqp_bm_multiref2(QM) = ripqp(QM, iconf = InputConfig(mode=:multi, refinement = :multiref), itol = InputTol(max_time=1200., ϵ_pdd32=1.0))
# ripqp_bm_zoom(QM) = ripqp(QM, iconf = InputConfig(refinement = :zoom), itol = InputTol(max_time=1200., ϵ_rbz = 1e-5))
# ripqp_bm_zoom2(QM) = ripqp(QM, iconf = InputConfig(refinement = :zoom), itol = InputTol(max_time=1200., ϵ_rbz = 1e-7))
# ripqp_bm_multizoom(QM) = ripqp(QM, iconf = InputConfig(mode=:multi, refinement = :multizoom), itol = InputTol(max_time=1200.))
# ripqp_bm_multizoom2(QM) = ripqp(QM, iconf = InputConfig(mode=:multi, refinement = :multizoom), itol = InputTol(max_time=1200., ϵ_pdd32=1.0))
...
save_problems(string(save_path, "/ripqp_mono_IPFK2_2"), ripqp_bm_classic)
