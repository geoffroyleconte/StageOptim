function convert_data(T, Qvals, Avals, c, c0, b, lvar, uvar)
    ...
    lvar_T = Array{T}(lvar)
    uvar_T = Array{T}(uvar)
    return Qvals_T, Avals_T, c_T, c0_T, b_T, lvar_T, uvar_T
end
...
function init_params(T, Qrows, Qcols, Qvals,  Arows, Acols, Avals, c, c0, b,
    ...
    Qvals_T, Avals_T, c_T, c0_T,
        b_T, lvar_T, uvar_T = convert_data(T, Qvals, Avals, c, c0, b, lvar, uvar)
    ϵ_pdd_T = T(1e-1)
    ϵ_rb_T = T(1e-1)
    ϵ_rc_T = T(1e-1)
    tol_Δx_T = T(tol_Δx)
