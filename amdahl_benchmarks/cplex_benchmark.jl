...
stats2 = cplex(qm2)  # compile code

function optimize_cplex(path_pb)
    problems = []
    i_max = 1000
    i = 1
    ...
    return solve_problems(cplex2, problems)
end
...
file_lp = jldopen(string(save_path, "/cplex_scaling_lp3.jld2"), "w")
file_lp["stats"] = problems_stats_lp
close(file_lp)

problems_stats_qp =  optimize_cplex(path_pb_qp)

file_qp = jldopen(string(save_path, "/cplex_scaling_qp3.jld2"), "w")
